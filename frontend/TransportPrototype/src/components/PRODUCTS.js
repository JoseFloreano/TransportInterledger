...
const PRODUCTS = ({ navigation }) => {
  ...
  // Recargar productos al volver a la pantalla
  useEffect(() => {
    const unsubscribe = navigation.addListener('focus', () => {
      loadProducts();
    });
    ...
    <View style={styles.container}>
      ...
      <View style={styles.content}>
          ...
          >
            {products.length === 0 ? (
              ...
              products.map((p, index) => (
                <View
                  key={p.id ? p.id.toString() : `product-${index}`} // key única garantizada
                ...
                >
                  ...
                  <TouchableOpacity
                    onPress={() =>
                      navigation.navigate('WRITE_NFC', { product: p })
                    }
                  >
                    <Text style={styles.actionLink}>write</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    onPress={() =>
                      navigation.navigate('VIEW_QR', { product: p })
                    }
                  >
                    <Text style={styles.actionLink}>view</Text>
                  </TouchableOpacity>
        ...
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate('INSERT_PRODUCTS')}
        >
          <Text style={styles.insertButtonText}>INSERT PRODUCT</Text>
        </TouchableOpacity>
      </View>
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#B8B8B8', paddingTop: 60 },
  centerContent: { justifyContent: 'center', alignItems: 'center' },
  loadingText: { marginTop: 10, fontSize: 14, color: '#2C2C2C' },
  backButton: { position: 'absolute', top: 70, left: 30, zIndex: 10 },
  backButtonText: { fontSize: 24, color: '#2C2C2C' },
  ...
  tableContainer: {
    ...
    borderRadius: 20,
    padding: 20,
    maxHeight: 500, // altura limitada para ScrollView
  },
  tableHeader: {
    flexDirection: 'row',
